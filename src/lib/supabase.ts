...
const supabaseUrl = 'https://lkodmymhhxlvqefodgdz.supabase.co'
const supabaseAnonKey = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imxrb2RteW1oaHhsdnFlZm9kZ2R6Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTcwMTE0MTEsImV4cCI6MjA3MjU4NzQxMX0.aCpgFP9CT0BTuho77Fx8vHRLs9zzazI9m_Ts17FdDzE'
  ...
  try {
    ...
    return {
      kpis: {
        ...
        studyStreak: Math.ceil(metrics.total_study_time_hours),
 ...
 */
export const checkLessonLockStatus = async (lessonId: string): Promise<LessonLockStatus> => {
  try {
    ...
    return {
      ...
      canDelete: !allStudentsCompleted, // Also prevent deletion when locked
      totalAssigned,
      totalCompleted,
      completionRate
    };
...
export const getLessonsWithLockStatus = async (tutorId: string): Promise<LessonWithAssignments[]> => {
  try {
    ...
    const lessonsWithLockStatus = await Promise.all(
      lessonsData.map(async (lesson): Promise<LessonWithAssignments> => {
    ...
    console.error('Error fetching lessons with lock status:', error);
    throw error;
  }
};